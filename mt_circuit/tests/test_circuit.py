...
def test_circuit_equivalence():
    ...
    qc.add_gate(name="RX", targets=[0,], angle=0.01)
    qc.add_gate(name="RY", targets=[0,], angle=0.02)
    qc.add_gate(name="RZ", targets=[0,], angle=0.03)

    u2 = unitary_group.rvs(2)
    u4 = unitary_group.rvs(4)
    qc.add_gate(name="u2", targets=[0,], matrix=u2)
    qc.add_gate(name="u4", targets=[0,1], matrix=u4)

    qc.add_gate(name="HPI", targets=[0,], angle=0.04)
