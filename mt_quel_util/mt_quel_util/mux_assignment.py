...
def get_multiplex_config(
    ...
    channel_to_quel_name: dict[str, str],
    ...
    for port_key, channel_list in port_channel_list.items():
        ...
        port_mux_info = multiplex_port(
            port_channel_to_frequency, LO_freq, NCO_step_freq, num_dac_channel, LO_sideband, constant
        )
    ...
    return MultiplexingResult(
        channel_to_dac_index=channel_to_dac_index,
        channel_to_residual_frequency=channel_to_residual_frequency,
        channel_to_pulse_bandwidth=channel_to_pulse_bandwidth,
        CNCO_setting=CNCO_setting,
        FNCO_setting=FNCO_setting,
    )
...
@pydantic.validate_call
def get_residual_frequency(
    freq_LO: FrequencyType,
    freq_CNCO: FrequencyType,
    freq_FNCO: FrequencyType,
    freq_target: FrequencyType,
    sideband: Literal["USB", "LSB", "Direct"],
) -> FrequencyType:
...
@pydantic.validate_call
def multiplex_port(
    channel_to_freq: dict[str, FrequencyType],
    freq_LO: FrequencyType,
    freq_NCO_step: FrequencyType,
    num_dac_channel: int,
    sideband: Literal["USB", "LSB", "Direct"],
    constant: InstrumentConstantQuEL,
) -> PortMultiplexing:
    freq_mean: FrequencyType = np.mean(list(channel_to_freq.values()))  # type: ignore
    freq_CNCO, _ = approximate_frequency_by_step(
        get_residual_frequency(freq_LO, 0 * GHz, 0 * GHz, freq_mean, sideband), freq_NCO_step
    )
    ...
    for dac_index in range(num_dac_channel):
        ...
        freq_FNCO, _ = approximate_frequency_by_step(
            get_residual_frequency(freq_LO, freq_CNCO, 0 * GHz, freq_dac_mean, sideband), freq_NCO_step
        )
    ...
    return PortMultiplexing(
        channel_to_dac=channel_to_dac,
        channel_to_residual_frequency=channel_to_residual_freq,
        channel_to_pulse_bandwidth=channel_to_pulse_bandwidth,
        CNCO_frequency=freq_CNCO,
        FNCO_frequency_dict=freq_FNCO_dict,
    )
