...
def _get_frequency_group_with_width(
    channel_to_frequency: dict[str, FrequencyType], width_max: FrequencyType
) -> tuple[list[list[str]], FrequencyType]:
    sorted_channel_freq_pair = sorted(channel_to_frequency.items(), key=lambda x: x[1])
    grouping: list[list[str]] = [
        [sorted_channel_freq_pair[0][0]],
    ]
    width_largest = 0 * GHz
...
def get_frequency_group(
    channel_to_frequency: dict[str, FrequencyType], accuracy: FrequencyType, num_dac_channel: int
) -> dict[str, int]:
    result: dict[str, int] = {}
