...
def modulate_waveform(channel_waveform: np.ndarray, frequency_modulate: FrequencyType, constant: InstrumentConstantQuEL) -> np.ndarray:
...
def demodulate_waveform(readout_waveform: np.ndarray, frequency_modulate: FrequencyType, constant: InstrumentConstantQuEL, acquisition_start_time: TimeType):
    assert(readout_waveform.ndim==1)
    ...
    phase_factor = 2*np.pi*(frequency_modulate["MHz"]/ADC_decimated_freq['MHz']) * (np.arange(len(readout_waveform)) + acquisition_sample_position)
    coef_factor = np.exp(-1j*phase_factor)
    corrected_readout_waveform = readout_waveform * coef_factor
    return corrected_readout_waveform

def modulate_averaging_window(averaging_window_coefficients: np.ndarray, frequency_modulate: FrequencyType, constant: InstrumentConstantQuEL):
    ADC_decimated_freq = constant.ADC_decimated_freq
    phase_factor = 2*np.pi*(frequency_modulate["MHz"]/ADC_decimated_freq["MHz"])*np.arange(len(averaging_window_coefficients))
    coef_factor = np.exp(1j*phase_factor)
...
def demodulate_averaged_sample(sample_list: np.ndarray, frequency_modulate: FrequencyType, constant: InstrumentConstantQuEL, acquisition_start_time: TimeType):
    ADC_decimated_freq = constant.ADC_decimated_freq
    acquisition_sample_position = acquisition_start_time["ns"] * ADC_decimated_freq["GHz"]
