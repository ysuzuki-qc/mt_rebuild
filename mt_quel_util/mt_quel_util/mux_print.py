...
def print_mux_config(channel_to_frequency: dict[str, FrequencyType], channel_to_device: dict[str, str], channel_to_port_index: dict[str, int], constant: InstrumentConstantQuEL, result: MultiplexingResult):
    for channel in channel_to_frequency:
        ...
        if LO_sideband == "LSB":
            print(f"frequency:  LO:{LO_freq['MHz']:8.2f} - CNCO:{CNCO_freq['MHz']:8.2f} - FNCO:{FNCO_freq['MHz']:8.2f} - RES:{RES_freq['MHz']:8.2f} = {(LO_freq-CNCO_freq-FNCO_freq-RES_freq)['MHz']:8.2f} MHz (target={channel_freq['MHz']:8.2f} MHz)")
