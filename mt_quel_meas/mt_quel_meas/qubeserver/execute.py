...
from mt_quel_meas.qubeserver.util import _boxport_to_device_and_port_index
...
class JobExecutorQubeServer:
    ...
    def _update_common_config(self, acquisition_config: AcquisitionConfigQubeServer) -> None:
        ...
        logger.info(f"job execute | set daq_synchronizatoin_delay | v: {acquisition_config.acquisition_synchronization_delay}")
    ...
    def _update_capture_point_list(self, capture_channel_to_adc_unit: dict[str, PhysicalUnitIdentifier], capture_channel_to_capture_point_list: dict[str, list[TimeType]], acquisition_duration: TimeType) -> None:
    ...
    def _do_measurement(self) -> None:
        ...
        packet.daq_trigger()
        packet.daq_stop()
        logger.info(f"job execute | daq | job sent")
        task = packet.send_future()
        
        # wait for response
        logger.info(f"job execute | daq | waiting...")
        start = time.time()
        task.result()
        end = time.time() - start
        ...
        # clear daq settings
        logger.info(f"job execute | daq clear")
    ...
    def _download_waveform(self, capture_channel_to_adc_unit: dict[str, PhysicalUnitIdentifier], capture_channel_to_capture_point: dict[str, list[TimeType]]) -> dict[str, np.ndarray]:
        ...
        for channel, capture_point_list in capture_channel_to_capture_point.items():
            ...
            logger.info(f"job execute | download waveform | ch: {channel} num_window: {len(capture_point_list)} waveform_shape: {waveform.shape}")
        return result
    ...
    def do_measurement(self, job: JobQubeServer) -> dict[str, np.ndarray]:
        ...
        # update Capture
        self._update_capture_point_list(job.capture_channel_to_adc_unit, job.capture_channel_to_capture_point_list, job.acquisition_config.acquisition_duration)
