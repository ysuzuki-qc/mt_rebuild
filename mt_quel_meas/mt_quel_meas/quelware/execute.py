...
try:
    ...
    from quel_ic_config import Quel1Box, Quel1BoxType  # pyright: ignore[reportMissingImports]
...
if __import_success:
    ...
    class QuEL1ManagerQuelware:
        ...
        def _update_waveform(
            self, ID_to_box_port_dac: dict[str, ChannelIdentifier], ID_to_waveform: dict[str, np.ndarray]
        ):
        ...
        def _update_NCO_frequency(
            self,
            ID_to_box_port_dac: dict[str, ChannelIdentifier],
            ID_to_NCO_frequency: dict[str, tuple[FrequencyType, list[FrequencyType]]],
        ):
        ...
        def _update_capture_point(
            self, ID_to_box_port_dac: dict[str, ChannelIdentifier], ID_to_capture_point: dict[str, list[TimeType]]
        ):
            for ID, capture_point_list in ID_to_capture_point.items():
                ...
                for index, capture_point in enumerate(capture_point_list):
                    ...
                    capture_point["ns"]
                    capture_param.sections.append(
                        CapSection(name=name, num_capture_word=(2048 + 512) // 4, num_blank_word=4 // 4)
                    )
                box = self.box_dict[channel.box]
                box.config_runit(channel.port, channel.dac, capture_param=capture_param)

        def _get_relevant_box_list(
            self, ID_to_box_port_dac: dict[str, ChannelIdentifier]
        ) -> list[tuple[str, Quel1Box, list, list]]:
        ...
        def do_measurement(self, job: JobQuelware) -> dict[str, np.ndarray]:
            ...
            for _, box, capture_units, awg_units in box_list:
                capture_task, awg_task = box.start_capture_by_awg_trigger(
                    capture_units, awg_units, synchronization_time
                )
