...
def _create_1Q_sequence(target_qubit_list: list[int]) -> Sequence:
...
def _create_1Q_frequency_dict(target_qubit_list: list[int]) -> dict[str, FrequencyType]:
...
def _create_1Q_frequency_shift_dict(target_qubit_list: list[int]) -> dict[str, FrequencyType]:
    frequency_dict: dict[str, FrequencyType] = {}
    role_list = ["qubit", "resonator"]
    for qubit_index in target_qubit_list:
        for role in role_list:
...
def _create_1Q_averaging_window_dict(target_qubit_list: list[int], constant: InstrumentConstantQuEL) -> dict[str, np.ndarray]:
...
def _create_1Q_translation_info(target_qubit_list: list[int], wiring_dict: dict, constant: InstrumentConstantQuEL) -> TranslationInfo:
    role_list = ["qubit", "resonator"]
...
def create_1Q_objects(target_qubit_list: list[int], wiring_dict: dict, constant: InstrumentConstantQuEL) -> tuple[Sequence, dict, dict, dict, TranslationInfo]:
    sequence = _create_1Q_sequence(target_qubit_list)
    channel_to_frequency = _create_1Q_frequency_dict(target_qubit_list)
    channel_to_frequency_shift = _create_1Q_frequency_shift_dict(target_qubit_list)
    channel_to_averaging_window = _create_1Q_averaging_window_dict(target_qubit_list, constant)
    translation = _create_1Q_translation_info(target_qubit_list, wiring_dict, constant)
