...
```python
import json
from mt_pulse import PulseFunction
...
pulse_load = PulseFunction.from_json_dict(json.loads(json_str))
print("json\n", json_str, "\n")
...
json
 {"name": "gaussian_drag", "pulse_expr": "Mul(Symbol('amplitude'), Add(Mul(Float('5.5451774444795623', precision=53), I, Symbol('drag'), Symbol('t'), Pow(Symbol('width'), Integer(-2)), exp(Mul(Integer(-1), Float('2.7725887222397811', precision=53), Pow(Symbol('t'), Integer(2)), Pow(Symbol('width'), Integer(-2))))), exp(Mul(Integer(-1), Float('2.7725887222397811', precision=53), Pow(Symbol('t'), Integer(2)), Pow(Symbol('width'), Integer(-2))))), exp(Mul(Float('1.0', precision=53), I, Symbol('phase'))))", "pulse_progress_time_ns": "Float('0.0', precision=53)", "pulse_start_time_ns": "Mul(Float('4.0', precision=53), Symbol('width'))", "pulse_end_time_ns": "Mul(Float('4.0', precision=53), Symbol('width'))"}
```
...
import sympy as sp
import matplotlib.pyplot as plt
from mt_pulse import PulseFunction, PulseFunctionLibrary
from mt_pulse.preset import gaussian

# create libs
pulse_lib = PulseFunctionLibrary()
...
pulse = PulseFunction(
    name="triangle",
    pulse_expr=shape,
    pulse_progress_time_ns=sp.Float(0),
...
pulse_lib_load = PulseFunctionLibrary.from_json_dict(json.loads(json_str))
